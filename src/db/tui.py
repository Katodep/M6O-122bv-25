...
def _update_student() -> None:
    print("\nОбновление записи")
    student_id = _read_int("Введите id для обновления: ")
    ...
    if not sex:
        sex = current[4]
    try:
        record = update_record(student_id, first_name, second_name, age, sex)
        print(f"Запись обновлена: {record}")
    except ValueError as exc:
        print(f"Ошибка: {exc}")

def _delete_student() -> None:
    student_id = _read_int("Введите id для удаления: ")
